...
```
We need:
  O(1) get() → fetch value & update frequency.
  O(1) put() → insert/update key, and if full, evict the least frequently used (and if tie, the LRU among them).

To do this efficiently:
  Track (value, frequency) per key → cache map.
  Track all keys with the same frequency → frequency map (freq → ordered set of keys).
  Track the minimum frequency present at any time.
```

Data Structures
```
cache: HashMap<key, (value, freq)>
frequencies: HashMap<freq, LinkedHashSet<keys>>
minf: smallest frequency currently in cache
...
get(key)
  ...
  Fetch (value, freq).
  Remove key from frequencies[freq].
  If freq == minf and set becomes empty → minf++.
  Insert key into frequencies[freq+1].
  Update cache[key] = (value, freq+1).
  Return value.

put(key, value)
  If capacity == 0 → do nothing.
  If key exists → update value & call get(key) to bump frequency. Done.
  If cache is full →
    Evict first key in frequencies[minf] (the LRU one).
    Remove it from both maps.
  Set minf = 1 (new key starts at freq 1).
  Insert (value, 1) into cache and add key into frequencies[1].
```
